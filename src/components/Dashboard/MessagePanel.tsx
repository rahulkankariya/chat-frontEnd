...
const MessagePanel: React.FC<MessagePanelProps> = ({ userName, avatar }) => {
  const [messages, setMessages] = useState<Message[]>([
    ...
    },
    {
      text: "Old message",
      fromMe: false,
      timestamp: Date.now() - 86400000 * 1, // 2 days ago
    },
  ]);
  ...
  return (
    <div className="flex flex-col flex-1 bg-[#e5ddd5] h-screen">
      {/* Header */}
      <div className="flex items-center gap-3 p-4 bg-[#075E54] text-white shadow">
      ...
      <div className="p-4 bg-white flex gap-2 border-t">
        ...
        <button
          onClick={sendMessage}
          className="bg-[#075E54] text-white px-5 py-2 rounded-full text-sm"
        >
          Send
        </button>
      </div>
